...
@section('title', 'Kreiraj korisnika')
    ...
    <section class="min-h-screen bg-slate-100 px-4 py-8 sm:px-6 lg:px-8">
        <div class="mx-auto max-w-3xl space-y-6">
            <div
                ...
                <h1 class="mt-2 text-3xl font-bold tracking-tight sm:text-4xl">Kreiraj korisnika</h1>
                <p class="mt-3 max-w-2xl text-sm text-slate-300 sm:text-base">
                    Dodaj novi korisnicki nalog popunjavanjem podataka ispod.
                </p>
            </div>
            ...
            <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
                <form method="POST" action="{{ route('createUserAPI') }}" class="space-y-6">
                    @csrf
                    <div>
                        <label for="name" class="mb-2 block text-sm font-semibold text-slate-700">Ime</label>
                        <input id="name" name="name" type="text" value="{{ old('name') }}"
                            ...
                            placeholder="Unesi puno ime" required>
                        @error('name')
                    ...
                    <div>
                        <label for="password" class="mb-2 block text-sm font-semibold text-slate-700">Lozinka</label>
