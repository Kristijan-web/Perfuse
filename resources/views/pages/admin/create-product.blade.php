    ...
    <section class="min-h-screen bg-slate-100 px-4 py-8 sm:px-6 lg:px-8">
        <div class="mx-auto max-w-5xl space-y-6">
            <div
                ...
                <h1 class="mt-2 text-3xl font-bold tracking-tight sm:text-4xl">Create product</h1>
                <p class="mt-3 max-w-2xl text-sm text-slate-300 sm:text-base">
                    Add a new product with its base details, available sizes, images, and optional discount settings.
                </p>
            </div>
                ...
                <form method="POST" action="{{ route('createProductAPI') }}" enctype="multipart/form-data" class="space-y-8">
                    ...
                    <div class="grid gap-6 lg:grid-cols-2">
                        <div class="lg:col-span-2">
                            <label for="title" class="mb-2 block text-sm font-semibold text-slate-700">Product title</label>
                        ...
                        <div>
                            <label for="brand_id" class="mb-2 block text-sm font-semibold text-slate-700">Brand</label>
                        ...
                        <div>
                            <label for="water_type_id" class="mb-2 block text-sm font-semibold text-slate-700">Water type</label>
                        ...
                        <div>
                            <label for="images" class="mb-2 block text-sm font-semibold text-slate-700">Product images</label>
                            ...
                            <p class="mt-2 text-sm text-slate-500">You can upload multiple images. Use the first image as the main one.</p>
                            @error('images')
                                <p class="mt-2 text-sm font-medium text-red-600">{{ $message }}</p>
                            @enderror
                            @error('images.*')
                    ...
                    <div class="rounded-3xl border border-slate-200 bg-slate-50 p-6">
                        <div class="flex items-start justify-between gap-4">
                            <div>
                                <h2 class="text-lg font-semibold text-slate-900">Available sizes</h2>
                                <p class="mt-1 text-sm text-slate-500">Choose one or more bottle sizes for this product.</p>
                            </div>
                        </div>
                    ...
                    <div class="rounded-3xl border border-slate-200 bg-slate-50 p-6">
                        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                            <div>
                                <h2 class="text-lg font-semibold text-slate-900">Discount settings</h2>
                                <p class="mt-1 text-sm text-slate-500">Enable discount only if this product should go on sale immediately.</p>
                            </div>
                        ...
                        <div id="discount-fields" class="{{ old('has_discount') ? '' : 'hidden' }} mt-5 grid gap-6 lg:grid-cols-3">
                            <div>
                                <label for="discount" class="mb-2 block text-sm font-semibold text-slate-700">Discount percentage</label>
                            ...
                            <div>
                                <label for="end_date" class="mb-2 block text-sm font-semibold text-slate-700">End date</label>
